...
====================================
A query builder that constructs SQL SELECT statements step-by-step. This is a
common pattern in ORMs and database libraries.

The SQL QueryBuilder shows how Builder naturally fits domain-specific languages.
Each method call adds a clause, and build() (or toSql()) assembles everything
into a valid SQL string.

Key characteristics:

- Fluent interface: Each method returns *this, allowing chaining.
- Optional parameters: You can omit any clause and the SQL remains valid.
- Type safety: The builder returns the final complex object (SqlQuery).
*/
